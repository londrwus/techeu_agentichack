...
export function forecastChart(el, opts = {}) {
  if (!window.echarts || !el) return { update() {}, dispose() {}, chart: null };
  el.classList.add('fc');
  ...
  function build() {
    ...
    const series = [
      { // 0 history
        ...
        areaStyle: { color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [{ offset: 0, color: 'rgba(100,116,139,.18)' }, { offset: 1, color: 'rgba(100,116,139,0)' }]) },
