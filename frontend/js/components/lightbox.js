...
import { api, esc, isNum, MODULE_META, ITEM_META } from '../lib.js';
...
function show(i) {
  ...
  state.spark?.setOption({ series: [{ markLine: { silent: true, symbol: 'none', animation: false, label: { show: false }, lineStyle: { color: '#F97316', width: 1.5, type: 'solid' }, data: [{ xAxis: state.i }] } }] });
}
...
function initMap(el) {
  ...
  if (!window.maplibregl) { el.innerHTML = `<div class="lb-nomap">Map library unavailable offline.</div>`; return; }
  const r = state.r;
...
function togglePlay() { state?.timer ? stopPlay() : startPlay(); }
function startPlay() {
