...
Source of truth: `design/orbit.pen` (Pencil). Screenshots: `design/01-overview.png`, `02-module-latte.png`, `03-rent-radar.png`, `04-mission-control.png`, `05-ask-orbit.png`. Tokens: `frontend/tokens.css` (Inter from Google Fonts). All frames are 1440×900. Build for 1440×900 first; below 1280 px it may stack.
...
3. **Bottom row** (fills the remaining height, gap 16):
   ...
   - **Satellite watchlist** card (fills the rest). Header: title + "16 regions · Sentinel-2", with a segmented control on the right: All / Crops / Water / Built. The control has bg `--bg`, padding 4, radius 10; the active item is white with `--shadow-seg`; items are 13px.
     - Map area: fills the card, radius 12, bg `--map-light`, overflow hidden. Background is the dotted world map `design/images/generated.png` (copy it to `frontend/assets/world-dots.png`). Position it as an absolutely placed, stretched image:
