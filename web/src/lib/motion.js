// Orbit motion layer. Page fade + staggered section rise, top progress bar for /api fetches,
...
export const SAFETY = 1500;
export const noAnim = el => { try { return getComputedStyle(el).animationName === 'none'; } catch { return false; } };
...
/** The router calls this on every navigation so the bar shows during a route swap. */
export function routeTick() {
  routeBusy = true; tick();
  setTimeout(() => { routeBusy = false; tick(); }, 80);
}
...
/** Fade a MapLibre canvas in once it has loaded. */
export function watchMap(map) {
  if (!map || REDUCED) return map;
  try {
